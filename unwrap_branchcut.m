function []=unwrap_branchcut(corr,  area_x, area_y);
...
%          
%
% Remake for use with InSAR (GMTSAR outputs) and octave by Andreas Steinberg, 27.6.2016 
...
[x,y,ampli]=grdread2('display_amp.grd');
ampli= ampli*1000;
[x,y, phase]=grdread2('phasefilt.grd');
[x,y, cor]=grdread2('corr.grd');
ampli = flipud(ampli);
phase = flipud(phase);
cor = flipud(cor);
...
%%
IM_mag=abs(ampli);                             %Magnitude image
IM_phase=(phase);  
...
%%  Set parameters
max_box_radius=9;                           %Maximum search box radius (pixels)
threshold_std=6;                            %Number of noise standard deviations used for thresholding the magnitude image
...
figure; 
imagesc(branch_cuts), colormap(gray), axis square, axis off, title('Branch cuts');
