...
% http://www.mathworks.com/access/helpdesk/help/techdoc/ref/netcdf_html
...
ncid = netcdf_open(file, 'NC_NOWRITE');
if isempty(ncid),
  return,
end
...
if (nvars==3),                        % new (v4) GMT netCDF grid file
  x=netcdf_getVar(ncid,0)';   
  y=netcdf_getVar(ncid,1)';
  z=netcdf_getVar(ncid,2)';
else
  if (nvars==6),                        % old (v3) GMT netCDF grid file
    [dimname, dimlen] = netcdf_inqDim(ncid,1);
    if (dimname=='xysize'),             % make sure it really is v3 netCDF
      xrange=netcdf_getVar(ncid,0)';
      yrange=netcdf_getVar(ncid,1)';
      z=netcdf_getVar(ncid,5);
      dim=netcdf_getVar(ncid,4)';
      pixel=netcdf_getAtt(ncid,5,'node_offset');
